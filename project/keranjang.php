...
// Fungsi untuk menyimpan informasi keranjang ke dalam database
function saveToDatabase($conn, $itemName, $price, $quantity) {
    $query = "INSERT INTO keranjang (item_name, price, quantity) VALUES (?, ?, ?)";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "sii", $itemName, $price, $quantity);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Simpan setiap item keranjang ke dalam database
    foreach ($cart as $item) {
        saveToDatabase($conn, $item['itemName'], $item['price'], $item['quantity']);
    }

    // Hapus keranjang setelah disimpan ke database
    unset($_SESSION['cart']);
