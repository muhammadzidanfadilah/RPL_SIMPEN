...
// Fungsi untuk menyimpan informasi pembelian ke dalam database
function savePurchaseToDatabase($conn, $itemName, $price, $quantity, $namaPembeli, $alamat, $nomorHP, $metodePembayaran) {
    $query = "INSERT INTO pembelian (item_name, price, quantity, nama_pembeli, alamat, nomor_hp, metode_pembayaran) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "siiiiis", $itemName, $price, $quantity, $namaPembeli, $alamat, $nomorHP, $metodePembayaran);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
}

// Setelah formulir checkout dikirim
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $cart = json_decode($_POST['cart'], true);
    $userInfo = json_decode($_POST['user_info'], true);

    // Simpan setiap item pembelian ke dalam database
    foreach ($cart as $item) {
        savePurchaseToDatabase(
            ...
            $item['price'],
            $item['quantity'],
            $userInfo['namaPembeli'],
            $userInfo['alamat'],
            $userInfo['nomorHP'],
            $userInfo['metodePembayaran']
        );
    }

    // Hapus keranjang setelah pembelian disimpan ke database
    unset($_SESSION['cart']);
